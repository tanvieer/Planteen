...
function changeTotalPrice(idt) { // single product total price
	...
	document.getElementById("totalProductPrice" + idt).innerHTML = quantity
			* selling_Price;
...
function clearCart() {
	...
	if (confirm("Are you sure, you want to clear cart?")) {
		...
		xmlhttp.onreadystatechange = function() {
			if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
				// jao delete kore asho
				alert("test cart clearing");
				$('#result').empty();
				document.getElementById('result').innerHTML = 'SUB-TOTAL: 0 BDT';
			}
		};
		xmlhttp.open("GET", url, true);
		xmlhttp.send();
...
function CartUpdate(pid, quantity) {
	var xmlhttp;
	var url = "	CartUpdateServlet?productId=" + pid + "&productQuantity="
			+ quantity;
	if (window.XMLHttpRequest) {
		xmlhttp = new XMLHttpRequest();
	} else {
